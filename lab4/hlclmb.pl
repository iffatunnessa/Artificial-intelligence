...
do(1):- retractall(state(_,_,_,_)),retract(id(_)), retract(max_val(_)),
	retract(threshold(_)).
do(2):- go_hcs.
do(3):- listing(state).
...
go_hcs:- write('Enter a state:'), read(S),
	 ...
	 gnrt_sucsr(L),retract(threshold(_)).
...
/* Exercise:
 ...
 Change it to accomodate restarting with a randomly generated state.
